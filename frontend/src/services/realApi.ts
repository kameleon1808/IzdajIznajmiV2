...
const mapListing = (data: any): Listing => {
  ...
  return {
    ...
    rating: Number(data.listing_rating_avg ?? data.listingRatingAvg ?? data.rating ?? 0),
    reviewsCount: Number(data.listing_rating_count ?? data.listingRatingCount ?? data.reviewsCount ?? data.reviews_count ?? 0),
